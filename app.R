...
ui <- dashboardPage(
  ...
  dashboardBody(
      fluidRow(
          box(title = "" ,  status = "primary", plotOutput("cycleA"), width = 700, height=500),
...
output$perst<- renderPlot({
    ...
    ggplot(pd_df, aes(Birth, Persistence)) + geom_point() +rm(list=ls())
source('./utility-functions.R')
#source('./processing.R')
persistence_diagrams <- readRDS('./perstdiag.rds')
ui <- dashboardPage(
  ...
  dashboardBody(
      fluidRow(
          box(title = "" ,  status = "primary", plotOutput("cycleA"), width = 700, height=500),
          box(title = "Persistence",  status = "primary", plotOutput("perst"))
